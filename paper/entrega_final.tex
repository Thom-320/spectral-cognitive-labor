...
\hypersetup{%
  hidelinks,
  pdftitle={Simetria del tablero y especializacion axial en Seeking the Unicorn},
  pdfauthor={Thomas Chisica, Sara Figueredo Laserna, Juan Sebastian Mora, Angel Amaya},
...
\title{Simetría del tablero y especialización axial\\ en \emph{Seeking the Unicorn}}

\author{%
  Thomas Chisica, Sara Figueredo Laserna, Juan Sebastián Mora y Ángel Amaya\\
  Programa de Matemáticas Aplicadas y Ciencias de la Computación (MACC)\\
  Universidad del Rosario, Bogotá, Colombia\\
  Teoría de Grafos 2026-I, Prof.\ Daniel Alfonso Bojacá Torres\\
  \texttt{\{thomas.chisica, sara.figueredo, juans.mora, angel.amaya\}@urosario.edu.co}}

\maketitle

\begin{abstract}
Este trabajo interpreta el experimento \emph{Seeking the Unicorn} como un problema de teoría espectral de grafos. El tablero, dotado de conectividad de rey, se modela como el producto fuerte $G = P_8 \boxtimes P_8$, y la división espacial del trabajo de cada díada se representa como una partición de $G$ inducida por frecuencias acumuladas de visita. El punto de partida es que la simetría diédrica $D_4$ del tablero cuadrado coincide con la degeneración $\lambda_2 = \lambda_3 \approx 0{,}4164$ del Laplaciano; por tanto, el eigenespacio de Fiedler es bidimensional y una bisección obtenida desde un vector numérico arbitrario no constituye una referencia geométrica bien definida. A partir de esta observación se comparan particiones humanas tardías contra referencias espectrales sensibles a la degeneración, y se define una señal geométrica temprana calculada sobre las primeras cinco rondas ausentes comunes de cada díada. En los análisis realizados sobre SODCL (45 díadas auditadas, 29 con partición dura válida), las díadas con orientación axial estable presentan una conductancia media observada $h_{\mathrm{obs}} = 0{,}142$ frente a $0{,}714$ en las díadas mixtas (Mann--Whitney $U=3$, $p = 3{,}07 \times 10^{-5}$), junto con mayor información mutua y mayor divergencia de Jensen--Shannon entre las distribuciones de visita de los jugadores. La señal geométrica temprana alcanza $\mathrm{AUC}_{\mathrm{LOOCV}} = 0{,}804$ y, combinada con las métricas oficiales tempranas, $0{,}860$. Un contraejemplo sobre $P_6 \boxtimes P_8$, donde el segundo autovalor es simple, es consistente con una explicación estructural basada en la simetría $D_4$ del tablero cuadrado y no en un artefacto numérico.
\end{abstract}

\begin{IEEEkeywords}
teoría de grafos, producto fuerte, Laplaciano, valor de Fiedler, conductancia, entropía, simetría del tablero, coordinación multiagente
\end{IEEEkeywords}

\section{Introducción}

\IEEEPARstart{L}{a} coordinación entre agentes que carecen de comunicación explícita es un problema importante en sistemas multiagente, ciencia cognitiva y análisis del comportamiento colectivo \cite{mesbahi2010,andrade2021,goldstone2024}. En la tarea \emph{Seeking the Unicorn}, dos participantes exploran simultáneamente un tablero compartido de $8\times 8$ celdas en busca de un objetivo cuya posición se desconoce. A lo largo de sucesivas rondas, muchas díadas reducen la redundancia de sus visitas y estabilizan convenciones espaciales, en particular divisiones de tipo izquierda--derecha o arriba--abajo \cite{andrade2021}. El fenómeno de interés no es solo que exista coordinación, sino que la geometría del entorno restringe y favorece ciertas formas de organización por encima de otras.

La pregunta que guía este trabajo no es si los humanos ``vencen'' a un algoritmo espectral, sino cómo debe formularse una comparación válida entre las particiones humanas observadas y la estructura espectral del tablero. Una comparación ingenua contra un único vector de Fiedler resulta insuficiente cuando el segundo autovalor no es simple, pues en ese caso el redondeo espectral depende de la base numérica elegida por la descomposición. En el tablero cuadrado del experimento esa dificultad es estructural y no marginal: la simetría del problema induce un eigenespacio de Fiedler degenerado y, con ello, una familia de particiones espectrales geométricamente distintas. Reproducir observaciones ya documentadas por el estudio original \cite{andrade2021} bajo una etiqueta espectral ingenua no sería, por tanto, una contribución matemática.

Con esa precaución, el artículo replantea el problema con más precisión. En primer lugar, se modela el tablero como el producto fuerte $P_8 \boxtimes P_8$ y se introduce una familia paramétrica de referencias espectrales, sensible a la simetría $D_4$, que distingue entre un redondeo ingenuo y las direcciones axiales dentro del eigenespacio degenerado. En segundo lugar, se define una señal geométrica temprana, extraída de las primeras rondas sin unicornio comunes a los dos jugadores, para estudiar si la especialización axial posterior deja rastros medibles antes de que la división del trabajo se haya consolidado. Con ambas capas, el fenómeno se formula como un problema de teoría de grafos con una pregunta empírica delimitada y con referencias matemáticamente coherentes.

El resto del documento se organiza así. La Sección~\ref{sec:problema} describe el problema y los datos. La Sección~\ref{sec:objetivos} fija los objetivos. La Sección~\ref{sec:marco} desarrolla el marco teórico necesario. Las Secciones~\ref{sec:modelamiento} y~\ref{sec:solucion} formulan el modelamiento y la solución propuesta. La Sección~\ref{sec:implementacion} documenta el desarrollo computacional reproducible. La Sección~\ref{sec:resultados} presenta los resultados numéricos. La Sección~\ref{sec:conclusiones} discute alcance, limitaciones y trabajo futuro.

\section{Descripción del problema}\label{sec:problema}

El problema aplicado se origina en el conjunto de datos \emph{Self-Organized Division of Cognitive Labor} (SODCL), asociado a \emph{Seeking the Unicorn} \cite{andrade2021}. Cada díada juega 60 rondas; en cada una, ambos participantes inspeccionan celdas del tablero sin comunicación explícita y deciden si el unicornio está presente o ausente. La cobertura conjunta, el solapamiento entre jugadores y la estabilidad de las regiones exploradas permiten observar la emergencia de una división del trabajo.

El análisis requiere distinguir con precisión las dos fuentes oficiales de datos, resumidas en la Tabla~\ref{tab:datasets}. El archivo \emph{performances.csv} contiene la trayectoria completa de las 45 díadas a lo largo de las 60 rondas (presentes y ausentes) y se reserva para evaluar la transferencia descriptiva a medidas de desempeño conductual. El archivo \emph{humans\_only\_absent.csv}, en cambio, conserva únicamente las rondas sin unicornio e incluye variables derivadas ($DLIndex$, \emph{Similarity}, \emph{Consistency}, \emph{Joint}); sobre este subconjunto se formulan todos los cálculos de partición, conductancia y señales geométricas. Esta distinción importa metodológicamente: las rondas ausentes permiten observar patrones de cobertura sin censura por terminación temprana, mientras que la tabla completa es necesaria para estudiar la asociación con desempeño posterior. En consecuencia, el proyecto debe formularse como un reanálisis sobre un subconjunto estructurado del experimento, y no como si cada díada aportara 60 rondas equivalentes a todas las métricas.
...
El problema se resume en dos preguntas complementarias:
\begin{enumerate}
...
\item \textbf{Pregunta inferencial.} ¿Puede extraerse de las primeras rondas ausentes una señal geométrica asociada con la especialización axial estable observada al final del experimento?
\end{enumerate}

Ambas preguntas están conectadas. La primera depende de la geometría del grafo; la segunda, de cómo esa geometría se manifiesta en la dinámica temprana de exploración. El problema no es solo calcular un corte espectral; también exige relacionar una estructura algebraica del tablero con una regularidad conductual observable en los datos.

\section{Objetivo general y objetivos específicos}\label{sec:objetivos}

\subsection{Objetivo general}

Modelar el experimento \emph{Seeking the Unicorn} mediante teoría de grafos para estudiar cómo la simetría del tablero y la degeneración del eigenespacio de Fiedler se relacionan con la emergencia de especialización espacial axial en díadas humanas y con señales geométricas observables desde rondas tempranas.
...
\item Consolidar una herramienta computacional reproducible y examinar la asociación entre la orientación inferida y medidas de desempeño conductual.
\end{enumerate}
...
El estudio original documentó que la mayoría de díadas humanas logra una división espacial del trabajo y que dicha coordinación se organiza alrededor de unas pocas configuraciones focales \cite{andrade2021}. Trabajos posteriores han situado este experimento dentro de una literatura más amplia sobre emergencia de roles especializados en grupos pequeños \cite{goldstone2024}; esto permite interpretar el fenómeno como un caso de \emph{role specialization} y no como una mera descripción empírica de particiones.

\subsection{Producto fuerte y geometría del tablero}

Sea $P_n$ el grafo camino con $n$ vértices. Dados dos grafos $G_1=(V_1,E_1)$ y $G_2=(V_2,E_2)$, su producto fuerte $G_1 \boxtimes G_2$ tiene como conjunto de vértices $V_1\times V_2$; dos vértices $(u_1,u_2)$ y $(v_1,v_2)$ son adyacentes si y solo si en cada coordenada los vértices coinciden o son adyacentes, y al menos una coordenada presenta adyacencia estricta \cite{hammack2011}. La conectividad tipo rey del tablero experimental coincide precisamente con esta construcción, por lo que el espacio de búsqueda se modela como
\[
G = P_8 \boxtimes P_8,
\]
...
La conductancia cuantifica la frontera de una partición del grafo, pero no indica por sí sola si esa partición separa de manera interpretable el trabajo de los dos jugadores. Para evaluar ese segundo aspecto usamos medidas elementales de teoría de la información. Si $Z$ es una variable discreta con distribución $p(z)$, su entropía de Shannon es
\begin{equation}
H(Z) = - \sum_z p(z)\log_2 p(z).
\label{eq:entropy}
\end{equation}
Si $P \in \{1,2\}$ denota la identidad del jugador y $C$ la celda visitada, usamos la información mutua
\begin{equation}
I(P;C) = H(P) + H(C) - H(P,C)
\label{eq:mutual-information}
\end{equation}
para medir cuánto se reduce la incertidumbre sobre el jugador al conocer la celda visitada \cite{cover2006}. También comparamos directamente las distribuciones de visita. Si $p_1$ y $p_2$ son las distribuciones marginales de los dos jugadores y $m=(p_1+p_2)/2$, la divergencia de Jensen--Shannon es
\begin{equation}
\operatorname{JSD}(p_1,p_2)
= H(m) - \frac{1}{2}H(p_1) - \frac{1}{2}H(p_2).
\label{eq:jsd}
\end{equation}
Esta cantidad mide qué tan separados quedan los territorios explorados por los jugadores \cite{lin1991}. Aquí $I(P;C)$ y $\operatorname{JSD}$ no reemplazan a la conductancia. Funcionan como una segunda lectura: una partición puede tener poca frontera y, además, hacer que la celda visitada revele con bastante claridad quién la está explorando.
...
\]
Esta etiqueta no representa el conjunto completo de estrategias focales documentadas, sino únicamente la especialización axial que la lente espectral puede caracterizar de forma inequívoca.
...
La solución tiene dos capas complementarias, ambas apoyadas en la misma representación del tablero y en el mismo conjunto de variables. La primera capa aborda la pregunta estructural mediante una familia paramétrica de baselines espectrales. La segunda capa aborda la pregunta inferencial mediante un esquema de clasificación temprana sobre la etiqueta $y_d$.
...
La solución completa sigue estas etapas:
\begin{enumerate}
...
El código fuente sigue una secuencia explícita de scripts numerados; cada uno produce artefactos persistentes usados por los pasos posteriores y por el manuscrito. La Tabla~\ref{tab:pipeline} resume el papel de cada uno.
...
\label{tab:pipeline}
\begin{tabular}{@{}llp{0.38\linewidth}p{0.30\linewidth}@{}}
\toprule
...
Cinco decisiones de diseño condicionan la interpretabilidad y la reproducibilidad del pipeline.

Primero, la representación interna del tablero como matriz de adyacencia densa de $64\times 64$ permite usar rutinas estables de descomposición espectral (\texttt{numpy.linalg.eigh}). Aunque una representación dispersa sería preferible para grafos mayores, la dimensión del problema lo hace innecesario y favorece la legibilidad del código.
...
Cuarto, los modelos de clasificación se entrenan bajo \emph{leave-one-out} con estandarización dentro del bucle de validación e imputación por la mediana de la muestra de entrenamiento. Esta separación reduce el riesgo de fuga de información; aun así, por el tamaño de la muestra, la AUC debe leerse como una estimación exploratoria.
...
\]
mientras que para el Laplaciano normalizado se obtiene $\mu_2 \approx 0{,}0702$, compatible con la cota de Cheeger \eqref{eq:cheeger}. La igualdad numérica $h_{\mathrm{LR}} = h_{\mathrm{TB}} = h_{\mathrm{best\,axis}}$ indica que dentro del eigenespacio degenerado existen direcciones axiales que alcanzan el mejor baseline observado en el barrido paramétrico. Por tanto, LR y TB no son artefactos de una elección arbitraria de base.
...
La Tabla~\ref{tab:comparacion} resume las medias por grupo y los contrastes de Mann--Whitney. Las diferencias entre díadas axiales y mixtas son consistentes entre variables y alcanzan significancia nominal: la conductancia observada es sustancialmente menor en el grupo axial, mientras que la eficiencia $\eta = h(\Sing)/h(\Sobs)$, la información mutua y la divergencia de Jensen--Shannon son mayores. Dado el tamaño muestral, estos contrastes se interpretan como evidencia exploratoria, no como estimaciones poblacionales definitivas. La Figura~\ref{fig:spec-comp} ilustra estas diferencias.
...
La eficiencia $\eta$ del grupo axial supera la unidad, lo que indica que la conductancia observada se aproxima al baseline axis-aligned $h = 22/210$ y supera una referencia numérica arbitraria de Fiedler. Esta observación no debe interpretarse como evidencia de que las díadas humanas optimicen globalmente la conductancia: la geometría del tablero privilegia cortes axiales, y las díadas estables observadas tienden a organizarse de manera compatible con esa estructura.
...
\subsection{Especialización informacional y entropía}\label{sec:res-entropia}

El análisis informacional ofrece una lectura sencilla de la división del trabajo. La conductancia cuantifica la frontera de la partición; $I(P;C)$ y $\operatorname{JSD}(p_1,p_2)$ evalúan si, a partir de las celdas visitadas, se puede distinguir el territorio de cada jugador. En el conjunto primario, las díadas axiales alcanzan $MI = I(P;C) = 0{,}839 \pm 0{,}264$ bits y $JSD = 0{,}842 \pm 0{,}261$, frente a $MI = 0{,}260 \pm 0{,}176$ bits y $JSD = 0{,}284 \pm 0{,}174$ en las díadas mixtas. Los contrastes de Mann--Whitney muestran diferencias claras entre ambos grupos ($MI$: $U=156$, $p = 4{,}23 \times 10^{-4}$; $JSD$: $U=157$, $p = 3{,}51 \times 10^{-4}$).

La Figura~\ref{fig:entropy} muestra que las métricas informacionales no aportan una dimensión completamente independiente: aumentan junto con la eficiencia espectral $\eta$ y separan a los mismos grupos. Ese resultado no identifica por sí solo un mecanismo causal de coordinación. Su aporte es descriptivo: las particiones axiales de baja conductancia también producen territorios más reconocibles para cada jugador.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{entropy_analysis.png}
\caption{Especialización informacional sobre el conjunto primario. Las díadas axiales muestran mayor información mutua y mayor divergencia de Jensen--Shannon que las mixtas; ambas medidas crecen con la eficiencia espectral $\eta$.}
\label{fig:entropy}
\end{figure}
...
La Tabla~\ref{tab:auc} resume las áreas bajo la curva ROC de los tres modelos de clasificación logística sobre el conjunto completo de 45 díadas (20 positivas, 25 negativas) y, como contraste, sobre el subconjunto de 32 díadas con etiqueta de orientación tardía válida (20 positivas, 12 negativas). En el conjunto completo, las díadas sin etiqueta axial tardía válida se tratan como no axiales para una auditoría predictiva conservadora. Sobre las 45 díadas, el modelo basado únicamente en la señal geométrica temprana $g_d$ alcanza $\mathrm{AUC}_{\mathrm{LOOCV}} = 0{,}804$ y supera al trío de métricas oficiales tempranas ($0{,}736$); la combinación de ambas familias de características eleva el desempeño a $0{,}860$. Dado el tamaño muestral y la ausencia de validación externa, estas AUC se interpretan como evidencia exploratoria de separabilidad temprana, no como desempeño predictivo generalizable. La Figura~\ref{fig:early} ilustra estas comparaciones.
...
Como diagnóstico complementario se evaluó el valor incremental de la conductancia tardía $h_{\mathrm{obs}}$ y de su recíproco normalizado $\eta$ frente al modelo oficial bajo \emph{leave-one-out}. Para la predicción del score promedio en rondas con unicornio presente, el modelo oficial alcanza $R^2_{\mathrm{LOOCV}} = 0{,}654$; añadir $h_{\mathrm{obs}}$ reduce el desempeño a $0{,}497$ y añadir $\eta$ a $0{,}575$. Resultados análogos se observan para precisión y conjunción. La interpretación aquí es directa: el aporte distintivo del proyecto no reside en una conductancia tardía que reemplace a las métricas oficiales, sino en la formulación espectral coherente, en la señal temprana $g_d$ y en la asociación consistente entre orientación inferida y desempeño conductual.

\section{Conclusiones}\label{sec:conclusiones}

El resultado principal es que la comparación espectral de las particiones humanas en \emph{Seeking the Unicorn} solo está bien planteada si se trata explícitamente la degeneración del eigenespacio de Fiedler. El tablero experimental se modeló como el producto fuerte $P_8 \boxtimes P_8$ y se caracterizó su espectro, documentando con precisión de máquina la degeneración $\lambda_2 = \lambda_3 \approx 0{,}4164$ asociada a la simetría $D_4$. Sobre esa base se distinguieron tres familias de referencias espectrales: el redondeo ingenuo, los cortes axiales LR y TB, y la familia paramétrica $\{S_\theta\}$; además, se mostró numéricamente que las direcciones axiales alcanzan la menor conductancia observada dentro del barrido evaluado. La partición observada de cada díada se definió por dominancia de frecuencias en una ventana ausente y se evaluó mediante conductancia, información mutua y divergencia de Jensen--Shannon. La etiqueta de orientación axial estable se construyó a partir de tres ventanas tardías y se evaluó mediante el índice de Jaccard. La señal geométrica temprana $g_d$ se diseñó como una proyección sobre dos plantillas axiales y se comparó con las métricas oficiales bajo LOOCV. Finalmente, el pipeline computacional quedó reproducible y la orientación inferida se cruzó con la tabla completa \emph{performances.csv} para una lectura descriptiva de su asociación con desempeño.

Los resultados apuntan en la misma dirección. Las díadas axialmente estables alcanzan conductancias compatibles con la referencia alineada con los ejes ($h_{\mathrm{obs}} \approx 0{,}142$, $\eta \approx 1{,}12$), mientras que las mixtas se mantienen lejos de esa referencia geométrica ($h_{\mathrm{obs}} \approx 0{,}71$, $\eta \approx 0{,}20$). Las métricas informacionales añaden una comprobación útil: en las díadas axiales, la celda visitada revela más sobre qué jugador la explora, y las distribuciones de visita de ambos jugadores quedan más separadas. Las diferencias observadas son consistentes en este conjunto de datos, aunque deben leerse con cautela por el tamaño muestral. La señal geométrica temprana, calculada sobre cinco rondas, alcanza $\mathrm{AUC}_{\mathrm{LOOCV}} = 0{,}804$ y, combinada con las métricas oficiales, $0{,}860$ sobre el conjunto completo de 45 díadas. El contraejemplo sobre $P_6 \boxtimes P_8$ es consistente con la lectura de que la degeneración tratada se asocia con la simetría $D_4$ del cuadrado y no aparece en la geometría rectangular probada. La orientación inferida se asocia con un mejor desempeño conductual posterior, especialmente en rondas con unicornio presente.

El alcance del trabajo está delimitado de manera explícita. Primero, la representación bipartita captura limpiamente las estrategias axiales del experimento, pero no las categorías \emph{ALL}, \emph{NOTHING} y \emph{RS}, que constituyen una fracción no despreciable de las díadas excluidas. Segundo, la conductancia tardía $h_{\mathrm{obs}}$ presenta una correlación de Spearman $\rho = 0{,}894$ con $DLIndex_{\mathrm{mean}}$ y no aporta valor predictivo incremental sobre las métricas oficiales para predecir desempeño; el aporte del proyecto reside, por tanto, en la formulación geométrica y en la señal temprana, no en una sustitución de las medidas existentes. Tercero, las métricas informacionales acompañan a la lente espectral, pero no prueban por sí solas un mecanismo causal de coordinación. Cuarto, la muestra es pequeña ($n=29$ en el conjunto primario, $n=45$ en la auditoría) y los resultados deben interpretarse como evidencia de plausibilidad de la lente espectral, no como inferencia causal; la asociación con desempeño se reporta de forma descriptiva, con la díada como unidad experimental, no la ronda.

Estas limitaciones sugieren tres líneas de trabajo futuro. La primera es teórica: explorar familias de plantillas más amplias que las dos direcciones axiales canónicas, derivadas explícitamente de las representaciones irreducibles de $D_4$ sobre el eigenespacio de Fiedler, para evaluar si existen orientaciones humanas no axiales sistemáticas que la presente formulación no captura. La segunda es metodológica: extender el análisis temporal a las 60 rondas completas, integrando rondas con unicornio presente bajo un esquema de censura adecuado, lo que permitiría modelar la cristalización de roles como un proceso dinámico y no únicamente como una clasificación binaria. La tercera es experimental: el diseño actual sugiere una predicción contrastable a partir de la teoría desarrollada. Si la degeneración del eigenespacio de Fiedler es la fuente estructural de la ambigüedad de roles axiales en el tablero cuadrado, modificar la simetría del entorno, por ejemplo mediante tableros rectangulares o cuadrados con obstáculos que rompan $D_4$, debería concentrar las estrategias alrededor de la orientación favorecida por la nueva geometría. Una validación empírica de este desplazamiento convertiría el reanálisis ofrecido en este trabajo en una agenda experimental concreta sobre cómo la estructura espectral del entorno se asocia con formas de coordinación multiagente.

En conjunto, el proyecto sugiere que la teoría espectral de grafos ayuda a reanalizar \emph{Seeking the Unicorn}: no reemplaza las métricas conductuales originales, pero aclara por qué ciertas divisiones espaciales son referencias geométricamente naturales y ofrece una señal temprana asociada con la estabilización posterior de roles axiales.

\section*{Disponibilidad del código y los datos}

Todo el código fuente, los datos crudos y los \emph{scripts} de reproducción del análisis se encuentran disponibles en el repositorio público
\href{https://github.com/Thom-320/grafos_proyecto}{\texttt{github.com/Thom-320/grafos\_proyecto}}.
El punto de entrada es \texttt{run.sh} (opción 11: \emph{pipeline} completo). Las dependencias quedan declaradas en \texttt{requirements.txt}.

\begin{center}
\qrcode[height=2.6cm,level=M,nolinks]{https://github.com/Thom-320/grafos_proyecto}\\[0.4em]
{\footnotesize\texttt{github.com/Thom-320/grafos\_proyecto}}
\end{center}
